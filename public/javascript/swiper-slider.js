...
jQuery(document).ready(function($){ 
		...
		$nectarSliders[i].swipeReset();
		 ...
		 if(navigator.userAgent.indexOf('Chrome') > 0) { 
		 	 if( jQuery(this).find('.swiper-slide:nth-child(2) video').length > 0 && jQuery(this).find('.swiper-container').attr('data-loop') == 'true') {
	          	...
	          	var webmSource2 = jQuery(this).find('.swiper-slide:nth-child(2) video source[type="video/webm"]').attr('src') + "?id="+Math.ceil(Math.random()*10000);
	...
	if(window.innerWidth > 690){
		$(window).resize(dynamicHeightSliders);
	} else {
		...
		$('.swiper-wrapper').each(function(){
			...
			$(this).find('.swiper-slide').each(function(){
				...
				if($(this).attr('data-y-pos') == 'top'){
					var $topHeight = ($contentHeight/2) < ((($sliderHeight+ $transparentHeader)/4) - 30) ? ((($sliderHeight+ $transparentHeader)/4) - ($contentHeight/2)) + 20 :  ($sliderHeight+ $transparentHeader)/8;
					$(this).find('.content').css('top', $topHeight + 'px');
				} 
				else if($(this).attr('data-y-pos') == 'middle') {
					$(this).find('.content').css('top', ((($sliderHeight + $transparentHeader)/2) - ($contentHeight/2)) + 'px');
				} 
				else {
					if($contentHeight > 180) { 
						$(this).find('.content').css('top', ((($sliderHeight + $transparentHeader)/2) - ($contentHeight/10)) +'px');
					} else {
						$(this).find('.content').css('top', ((($sliderHeight + $transparentHeader)/2) + ($contentHeight/9))  + 'px');
					}
				}
			});
		});
	...
	$('.slider-down-arrow').click(function(){
		...
		if(!$('body').hasClass('mobile')){
			$('body,html').stop().animate({
				scrollTop: $topDistance - $('#header-space').height() + shrinkNum + headerPadding*2 + $offset + 1
			},1000,'easeInOutCubic')
		} else {
			$('body,html').stop().animate({
				scrollTop: parseInt($topDistance) + parseInt($currentSlider.offset().top) + 1
			},1000,'easeInOutCubic')
		}
		return false;
	});
